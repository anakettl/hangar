...
- Exemplo: `V2__create_users_table.sql`

🗄️ Padrões de Banco

Adotamos como padrão nomear explicitamente as constraints no SQL.
Convenções:
- `pk_` + `tabela`
- `uk_` + `tabela` + `campo`
...
```
Isso facilita manutenção futura (`DROP/ALTER CONSTRAINT`) e deixa o schema mais legível.
